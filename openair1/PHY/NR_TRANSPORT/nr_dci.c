...
FloatPair getFloatPair(Bit bit) {
    FloatPair pair;
    if (bit == BIT_1) {
        pair.first = 0.2500;
        pair.second = 1.7500;
...
// 函數來將FloatPair轉換成bit sequence
unsigned char getBitSequence(FloatPair pair) {
    if (pair.first == 0.2500 && pair.second == 1.7500) {
        return 0b0011;
    } else if (pair.first == 1.7500 && pair.second == 0.2500) {
        return 0b1100;
    }
    return 0; // 若沒有匹配，傳回0（代表錯誤情況）
}

// 函數來將bit sequence轉換成二進位字串
...
void nr_generate_dci(PHY_VARS_gNB *gNB,
  ...
  for (int d=0;d<pdcch_pdu_rel15->numDlDci;d++) {
    ...
    // printf("AggregationLevel = %d :",dci_pdu->AggregationLevel);

#ifdef SEMANTIC_CODING_DEBUG
      printf("\n");
      printf("Encoded Bit: 1\n");
      printf("Encoder:\n");

      Bit bitChoice = BIT_1; // bit 1
      FloatPair selectedPair = getFloatPair(bitChoice);
      printf("Transmitted FP: [%.4f, %.4f]\n", selectedPair.first, selectedPair.second);

      unsigned char bitSequence = getBitSequence(selectedPair);
      printf("Binarizer:\n");
      printf("Transmitted bit sequence: ");
      ...
      // encoder_output[0] = dci_table[target_dci] & 0xFFFFFFFF;
      // encoder_output[0] = dci_table[target_dci] & 0xFFFFFFFF;// 0x0000000
      // encoder_output[0] = 0b1100 & 0xFFFFFFFF;
      encoder_output[0] = (bitSequence & 0xF) << 28;
...
[6]->0x%08x \t [7]->0x%08x \t [8]->0x%08x \t [9]->0x%08x\t [10]->0x%08x\t [11]->0x%08x\n",
    ...
    //   int16_t real = mod_dci[i << 1];
    //   int16_t imag = mod_dci[(i << 1) + 1];
    //   printf("Modulated complex signal i %d: %d + %dj\n", i, real, imag);
    // }
    // for (int i=0; i<encoded_length>>1; i++)
    ...
    for(int symbol_idx = 0; symbol_idx < pdcch_pdu_rel15->DurationSymbols; symbol_idx++) {
      ...
      for (int reg_count = 0; reg_count < num_regs; reg_count++) {
        ...
        // int index = symbol_idx * num_regs + reg_count;
        // if (index < (encoded_length >> 1)) {
        //   int16_t real = mod_dci[index << 1];
        //   int16_t imag = mod_dci[(index << 1) + 1];
        //   printf("Resource mapped complex signal symbol %d reg %d: %d + %dj\n", symbol_idx, reg_count, real, imag);
        // }
